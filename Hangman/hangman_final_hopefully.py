...
class Game:
    ...
    def list_view(self):
        for letter in self.secret_word:
            ...
            # print(secret_list)
        for blanks in range(0, len(self.secret_word)):
    ...
    #             elif user_guess not in self.user_guess_list:
    #                 self.attempts -= 1
    ...
    def run(self):
        ...
        if guessed_letter in self.user_guesses:
            print("You already guessed this letter, try something else.")

        else:
            self.attempts -= 1
            self.user_guesses.append(guessed_letter)
            if guessed_letter in self.secret_word_list:
                print("Nice guess!")
                if self.attempts > 0:
                    print("You have ", self.attempts, " attempts left!")
                letter_index = 0
                for letter in self.user_guess_list:
                    if letter == guessed_letter:
                        self.display_list[letter_index] = letter
                    letter_index += 1
                self.list_view()
            else:
                print("Oops! Try again.")
